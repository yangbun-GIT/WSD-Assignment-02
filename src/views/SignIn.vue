        ...
        <form v-else key="signup" @submit.prevent="handleSignup">
          ...
          <div class="input-group">
            <input
                v-model="password"
                type="password"
                placeholder="비밀번호 설정 (TMDB API Key)"
                required
            />
          </div>
          <div class="input-group">
...
const handleLogin = async () => {
  try {
    await axios.get(
        `https://api.themoviedb.org/3/configuration?api_key=${password.value}`
    )
...
.auth-wrapper::before {
  ...
  width: 100%;
  height: 100%;
  /* 중앙은 약간 투명하게(0.5), 바깥쪽으로 갈수록 거의 완전 검정색(0.95)으로 어두워짐 */

<template>
  <div class="movie-row">
    ...
    <div class="slider-wrapper" @mouseenter="showControls = true" @mouseleave="showControls = false">
      ...
      <div ref="slider" class="slider" @scroll="checkScroll" @wheel.prevent="onWheel">
        <MovieCard
...
defineEmits(['card-click']) // [추가] 이벤트 정의
...
const onWheel = (e: WheelEvent) => { if (slider.value) slider.value.scrollLeft += e.deltaY + e.deltaX }
...
.slider { display: flex; gap: 10px; overflow-x: auto; padding: 10px 0; scroll-behavior: smooth; scrollbar-width: none; }
